...
import { makeAutoObservable, observable, toJS, runInAction, action } from 'mobx'
import words from '../words.json'
import wordlist from '../wordlist.json'
...
class PuzzleStore {
  ...
  lastGuessRating: string = '';
  lastGuessScore: number = 0;
  ...
  addGuessResult(rating: string, score: number) {
    this.ratings[this.currentGuess - 1] = rating;
    this.scores[this.currentGuess - 1] = score;
  }
  ...
  async submitGuess() {
    if (this.isValidGuess()) {
        // Clear the last guess rating/score before getting new ones
        // runInAction(() => {
        //   this.lastGuessRating = undefined;
        //   this.lastGuessScore = undefined;
        // });

        // First, rate the guess
        const ratingResponse = await fetch('/api/rateGuess', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            guess: this.guesses[this.currentGuess],
            currentGuess: this.currentGuess,
            previousGuesses: this.guesses.slice(0, this.currentGuess),
            word: this.word
          })
        });
        ...
        runInAction(() => {
            // Store the current rating in the permanent array
            this.guessRatings[this.currentGuess] = ratingData.rating;
            this.guessScores[this.currentGuess] = ratingData.score;
        
            // Then update for the current row
            this.lastGuessRating = ratingData.rating;
            this.lastGuessScore = ratingData.score;
          });

          // Increment currentGuess or handle game end
      if (this.currentGuess >= this.maxGuesses - 1) {
        ...
        // Then proceed with existing guess validation and color calculation
        const colorsResponse = await this.fetchGuessColors(); 
